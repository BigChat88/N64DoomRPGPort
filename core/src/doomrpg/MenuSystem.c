...
void MenuSystem_free(MenuSystem_t* menuSystem, boolean freePtr)
{
	...
	DoomRPG_freeImage(menuSystem->doomRpg, &menuSystem->imgN64Pad);
	DoomRPG_freeImage(menuSystem->doomRpg, &menuSystem->imgAboutBg);
	menuSystem->imgBG = NULL;
	if (freePtr) {
		SDL_free(menuSystem);
	}
...
void MenuSystem_back(MenuSystem_t* menuSystem)
{
	if (menuSystem->oldMenu != -1) {
		...
		/* N64 port: the option menus defer their FlashRAM write to menu exit
		 * (a save per toggle stutters the audio mixer).  The "Back" item does
		 * it in Menu_select; do the same here for the B-button exit path. */
		switch (menuSystem->menu) {
			case MENU_VIDEO:      case MENU_INGAME_VIDEO:
			case MENU_SOUND:      case MENU_INGAME_SOUND:
			case MENU_CONTROLLER: case MENU_INGAME_CONTROLLER:
...
{
	if (menuSystem->type == 5 || menuSystem->type == 8) {
		// type 8 (image help) has no items: the scroll math is a safe no-op
...
void MenuSystem_paint(MenuSystem_t* menuSystem)
{
	...
	if (MenuSystem_checkMenu(menuSystem) || (menuSystem->menu != 0 && menuSystem->paintMenu))
	{
		...
		// N64 port: MENUTYPE_IMAGE -- a single full-screen picture (the Help
		// controller diagram), no item list, works in- and out-of-game.  Draw
		// it edge to edge over the whole framebuffer: the DoomRPG_* primitives
		// are displayRect-relative, so offset by -displayRect.x/y to reach
		// screen (0,0) and bypass the play-area overscan margin for this
		// static info screen (the image is authored 320x240).
		if (menuSystem->type == 8) {
			int fx = -doomCanvas->displayRect.x;
			int fy = -doomCanvas->displayRect.y;
			DoomRPG_fillRect(doomRpg, fx, fy, doomCanvas->clipRect.w, doomCanvas->clipRect.h);
			if (menuSystem->imgBG) {
				DoomCanvas_drawImage(doomCanvas, menuSystem->imgBG, fx, fy, 0);
			}
			DoomRPG_setFontColor(menuSystem->doomRpg, 0xffffffff);
			return;
		}

		// N64 port: About screen with a brick background -- fill the whole
		// framebuffer with the image (offset past the overscan margin) and let
		// the text draw on top.  No animated 3D scene / grid behind it.
		if ((menuSystem->menu == MENU_MAIN_HELP_ABOUT || menuSystem->menu == MENU_INGAME_HELP_ABOUT)
			&& menuSystem->imgBG == &menuSystem->imgAboutBg) {
			int fx = -doomCanvas->displayRect.x;
			int fy = -doomCanvas->displayRect.y;
			DoomRPG_fillRect(doomRpg, fx, fy, doomCanvas->clipRect.w, doomCanvas->clipRect.h);
			DoomCanvas_drawImage(doomCanvas, menuSystem->imgBG, fx, fy, 0);
		...
		else if (menuSystem->menu >= MENU_INGAME) {
			// N64 port: the Status screen carries a long stat list.  Let it use the
			// whole area between the HUD top/bottom bars (like the full-height
			// About screen) instead of a screenRect-sized centred panel, so far
			// fewer rows are pushed under the scrollbar.  Per-line content and
			// font are unchanged -- only the visible row count grows.
			if (menuSystem->menu == MENU_INGAME_STATUS) {
				menuSystem->maxItems = (doomCanvas->displayRect.h - doomRpg->hud->statusTopBarHeight - doomRpg->hud->statusBarHeight) / 12;
				i2 = doomRpg->hud->statusTopBarHeight;
			}
			else {
		...
		// N64 port: the About text is wrapped wider than the old 128px phone
		// column -- centre its (left-aligned) text column + scrollbar as one
		// block in the screen.
		if (menuSystem->menu == MENU_MAIN_HELP_ABOUT || menuSystem->menu == MENU_INGAME_HELP_ABOUT) {
			int sectionW = 208;
			i101 = (doomCanvas->displayRect.w - sectionW) / 2;
			scrollX = i101 + sectionW;
		}
		if (menuSystem->maxItems > 0 && menuSystem->numItems > menuSystem->maxItems) {
			DoomCanvas_drawScrollBar(doomCanvas, scrollX, i2, menuSystem->maxItems * 12, menuSystem->scrollIndex, menuSystem->scrollIndex + menuSystem->maxItems, menuSystem->numItems);
		}
...
void MenuSystem_startup(MenuSystem_t* menuSystem) {
	...
	/* N64 port: overlay assets (tools/overlay_zip.py merges assets/overlay/ into
	 * DoomRPG.zip).  Probe for each entry first so a missing asset degrades
	 * gracefully instead of aborting in DoomRPG_createImage():
	 *   n64pad.bmp  -> controller diagram for the Help screen (else text list)
	 *   aboutbg.bmp -> brick background for the About screen (else the logo) */
	menuSystem->imgN64Pad.imgBitmap = NULL;
	menuSystem->imgAboutBg.imgBitmap = NULL;
	{
		int sz;
		byte* probe;

		probe = readZipFileEntry("n64pad.bmp", &zipFile, &sz);
		if (probe && sz > 0) {
			SDL_free(probe);
			DoomRPG_createImage(menuSystem->doomRpg, "n64pad.bmp", false, &menuSystem->imgN64Pad);
		}

		probe = readZipFileEntry("aboutbg.bmp", &zipFile, &sz);
		if (probe && sz > 0) {
			SDL_free(probe);
			DoomRPG_createImage(menuSystem->doomRpg, "aboutbg.bmp", false, &menuSystem->imgAboutBg);
		}
	}
