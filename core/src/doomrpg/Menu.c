...
/*
 * Menu_setAbout -- the "About" screen (MENU_*_HELP_ABOUT).
 *
 * This is the original help.txt HELP/ABOUT + LEGALS text with the control
 * sections dropped (the N64 button layout lives on the separate "Help" screen,
 * MENU_*_HELP) and an N64 port credit spliced in.  Hardcoded rather than read
 * from help.txt so it does not depend on the BREW asset text.  A leading 0x80
 * byte is the font's section-divider glyph (help.txt writes it as '~').
 */
void Menu_setAbout(Menu_t* menu)
{
	static const char* const lines[] = {
		"\x80\x80\x80\x80\x80\x80 ABOUT \x80\x80\x80\x80\x80\x80",
		"In DOOM RPG you will fight",
		"monsters, solve puzzles and",
		"interact with characters in",
		"response to a crisis at a",
		"research station on Mars.",
		"",
		"\x80\x80\x80\x80\x80\x80 LEGALS \x80\x80\x80\x80\x80\x80",
		"(c) 2005 Id Software, Inc.",
		"All rights reserved.",
		"(r) 1993-2005 Id Software,",
		"Inc. Published by (c) JAMDAT",
		"Mobile Inc. under license",
		"from Id Software, Inc.",
		"DOOM and ID are registered",
		"trademarks of Id Software,",
		"Inc. JAMDAT, JAMDAT Mobile",
		"and the bubbles logo are",
		"trademarks or registered",
		"trademarks of (c) JAMDAT",
		"Mobile Inc. in the United",
		"States and / or other",
		"countries / regions.",
		"Fountainhead Entertainment",
		"and the FE logo are",
		"trademarks of Fountainhead",
		"Entertainment, Inc. All",
		"other trademarks are the",
		"trademarks of their",
		"respective owners and used",
		"with permission.",
		"",
	...
	for (i = 0; i < (int)(sizeof(lines) / sizeof(lines[0])); i++) {
		// left-aligned; the whole column + scrollbar is centred in MenuSystem_paint
		MenuItem_Set(&menuSystem->items[menuSystem->numItems++], (char*)lines[i], 0, 0);
	}
}

/*
 * Menu_setHelp -- the "Help" screen (MENU_*_HELP): a full-screen diagram of the
 * N64 controller with each button's in-game action.  If the n64pad.bmp overlay
 * asset is missing (DoomRPG.zip not rebuilt with tools/overlay_zip.py) it falls
 * back to a plain scrollable text list.
 */
void Menu_setHelp(Menu_t* menu)
{
	MenuSystem_t* menuSystem = menu->doomRpg->menuSystem;

	if (menuSystem->imgN64Pad.imgBitmap) {
		menuSystem->type = 8; // MENUTYPE_IMAGE: draw imgBG full-screen, no items
		menuSystem->imgBG = &menuSystem->imgN64Pad;
		return;
	}

	// Fallback: the image asset is not in the archive.
	static const char* const lines[] = {
		"\x80\x80 CONTROLS \x80\x80",
		"",
		"D-Pad  move/turn",
		"Stick  move/turn",
		"C L/R  strafe",
...
void Menu_textVolume(Menu_t* menu, int volume)
{
	...
	// N64 port: "Music" item inserted at index 2, so Volume moved 2 -> 3
	itemId = (menu->doomRpg->doomCanvas->sndFXOnly) ? 2 : 3;
...
void Menu_initMenu(Menu_t* menu, int i)
{
	...
	switch (i) {
		...
		case MENU_MAIN: {
			...
			MenuItem_Set(&menuSystem->items[menuSystem->numItems++], "Help      ", 2, 0);
			MenuItem_Set(&menuSystem->items[menuSystem->numItems++], "About     ", 2, 0);
			/* No "Exit" on N64 -- there is nowhere to quit to. */
			break;
		}

		case MENU_MAIN_HELP_ABOUT: { // "About" (control help moved to MENU_MAIN_HELP)
			// N64 port: brick background if the overlay asset is present,
			// otherwise the main-menu logo.
			menuSystem->imgBG = menuSystem->imgAboutBg.imgBitmap
				? &menuSystem->imgAboutBg : &menuSystem->imgLogo;
		...
		case MENU_MAIN_HELP: { // N64 controller diagram
			menuSystem->oldMenu = MENU_MAIN;
			Menu_setHelp(menu);
			break;
		}
		...
		case MENU_INGAME_OPTIONS: {
			...
			// N64 port: Bindings/Mouse dropped (dead); Controller kept for the
			// Rumble Pak toggle.
			MenuItem_Set(&menuSystem->items[menuSystem->numItems++], "Controller", 0, (i == MENU_INGAME_OPTIONS) ? true : false);
		...
		case MENU_INGAME_HELP_ABOUT: { // "About"
			strncpy(menu->doomRpg->hud->logMessage, "About", MS_PER_CHAR);
			// N64 port: brick background if present; else nothing (never inherit
			// the Help controller image).
			menuSystem->imgBG = menuSystem->imgAboutBg.imgBitmap ? &menuSystem->imgAboutBg : NULL;
			menuSystem->oldMenu = MENU_INGAME;
		...
		case MENU_INGAME_HELP: { // N64 controller diagram
		...
		case MENU_INGAME: {
			...
			MenuItem_Set(&menuSystem->items[menuSystem->numItems++], "Help", 0, MENU_INGAME_HELP);
			MenuItem_Set(&menuSystem->items[menuSystem->numItems++], "About", 0, MENU_INGAME_HELP_ABOUT);
		...
		case MENU_DEVELOPER_VARS: {
			...
			MenuItem_Set(&menuSystem->items[menuSystem->numItems++], "r_perf", 0, 0);
			break;
		}
		...
		case MENU_DEVELOPER: {
			...
			/* N64 port: trimmed to the items that do something useful here.
			 * Dropped: Store0-3 (out-of-context item vendor), Force Error
			 * (intentional abort), Print Monsters / Print Memory (stdout
			 * console no-ops -- there is no console on N64). */
			MenuItem_Set(&menuSystem->items[menuSystem->numItems++], "Back", 0, 0);
		...
		case MENU_INGAME_VIDEO: {
			...
			/* N64 port: FullScreen / VSync / IntScaling / Resolution / Softkeys
			 * were SDL-desktop options with no effect here -- removed.  Only the
			 * floor/ceiling texture toggle does anything on N64. */
			MenuItem_Set(&menuSystem->items[menuSystem->numItems++], "Back", 0, 0);
		...
		case MENU_INGAME_SOUND: {
			...
			// N64 port: per-level background music (assets/doomOST) toggle
			MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "Music:",
				menu->doomRpg->doomCanvas->musicEnabled ? "on" : "off", 0, 0);
		...
		// N64 port: trimmed to the Rumble Pak toggle (Bindings/Mouse removed).
		case MENU_CONTROLLER:
		case MENU_INGAME_CONTROLLER: {
			if (i == MENU_INGAME_CONTROLLER) {
			...
			MenuItem_Set(&menuSystem->items[menuSystem->numItems++], "Back", 0, 0);
			if (PD_RumbleSupported()) {
				MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "Rumble:",
					menu->doomRpg->doomCanvas->vibrateEnabled ? "on" : "off", 0, 0);
			}
			else {
				/* N64 port: no Rumble Pak detected on port 1 -- force the setting
				 * off and lock the row (flag 1 = cursor skips it, cannot toggle). */
				menu->doomRpg->doomCanvas->vibrateEnabled = false;
				MenuItem_Set2(&menuSystem->items[menuSystem->numItems++], "Rumble:", "off", 1, 0);
			}
			break;
		}
	...
	switch (menuId)
	{
		...
		case MENU_MAIN: {
			if (itemId == 0) { // Start Game
				...
				return MENU_MAIN_HELP; // Help (controller diagram)
			}
			else if (itemId == 3) {
				return MENU_MAIN_HELP_ABOUT; // About
			}
			else {
				return MENU_NONE;
			}
		...
		case MENU_DEVELOPER_VARS: {
			switch (itemId) {
			case 0:
				/* N64 port: persist on exit only (FlashRAM write stutters audio). */
				Game_saveConfig(menu->doomRpg->game, 0);
				return menuSystem->oldMenu;
			case 1:
			...
			case 9:
				...
				return MENU_DEVELOPER_VARS;
			case 10:
				doomCanvas->perfOverlay = !doomCanvas->perfOverlay;
				SDL_snprintf(text, sizeof(text), "r_perf %s", doomCanvas->perfOverlay ? "on" : "off");
				Hud_addMessage(menu->doomRpg->hud, text);
				doomCanvas->skipCheckState = true;
				return MENU_DEVELOPER_VARS;
			}
		...
		case MENU_DEVELOPER: {
			...
			case 1:
				return MENU_DEVELOPER_VARS;
			case 2:
				/* N64 port: the speed test renders the live scene, so it needs a
				 * map loaded (render->nodes).  The Debug menu is reachable from
				 * the main menu too, where there is no BSP and Render_walkNode()
				 * would dereference NULL (crash at Render.c Render_cullBoundingBox). */
				if (doomRpg->render->nodes == NULL) {
					Hud_addMessage(doomRpg->hud, "Load a map first");
					return MENU_DEVELOPER;
				}
				DoomCanvas_startSpeedTest(doomCanvas, false);
				return MENU_NONE;
			/* N64 port: Store0-3 / Force Error / Print Monsters / Print Memory
			 * items removed from MENU_DEVELOPER -- see the menu setup. */
			}
			break;
		}
		...
		case MENU_INGAME_VIDEO:
			...
			if (itemId == 0) {
				/* N64 port: persist on menu exit only.  A FlashRAM write per
				 * toggle blocks the CPU long enough to underrun the audio
				 * mixer, which clicks/drops the sound. */
				Game_saveConfig(menu->doomRpg->game, 0);
				return menuSystem->oldMenu;
			}
			else if (itemId == 1) { // Floor/Ceiling textures
		...
		case MENU_INGAME_SOUND:
			...
			// N64 port item layout: 0 Back, 1 Sound, 2 Music, 3 Volume
			if (itemId == 0) { // Back
				// Apply changes
			...
			if (itemId == 1) {
				...
				if (doomRpg->sound->soundEnabled) {
					...
					MenuItem_Set2(&menuSystem->items[3], "Volume:", "", 0, 0);

					SDL_snprintf(menuSystem->items[3].textField2,
						sizeof(menuSystem->items[3].textField2), "%d%%", (doomRpg->sound->volume * 100) / 100);
				}
				else {
					strncpy(menuSystem->items[itemId].textField2, "off", sizeof(menuSystem->items[itemId].textField2));
					Sound_stopSounds(doomRpg->sound);
					Sound_freeSounds(doomRpg->sound);
					MenuItem_Set(&menuSystem->items[3], NULL, 0, 0);
				}
			}
			else if (itemId == 2) { // Music (per-level background music)
				menu->doomRpg->doomCanvas->musicEnabled ^= true;
				PD_LevelMusicEnable(menu->doomRpg->doomCanvas->musicEnabled);
				strncpy(menuSystem->items[itemId].textField2,
					menu->doomRpg->doomCanvas->musicEnabled ? "on" : "off", sizeof(menuSystem->items[itemId].textField2));
			}
			else if (itemId == 3) {
		...
		// N64 port: Rumble Pak toggle.  Turning it ON fires a short test buzz
		// so you can confirm the pak is working.
		case MENU_CONTROLLER:
		case MENU_INGAME_CONTROLLER:
			...
			else if (itemId == 1 && PD_RumbleSupported()) { // Rumble (locked row when no pak)
				doomCanvas->vibrateEnabled ^= true;
				strncpy(menuSystem->items[itemId].textField2,
					doomCanvas->vibrateEnabled ? "on" : "off",
					sizeof(menuSystem->items[itemId].textField2));
				if (doomCanvas->vibrateEnabled) {
					DoomCanvas_vibrate(doomCanvas, 150); // short test buzz
				}
				/* N64 port: saved on Back only (itemId 0) -- see MENU_VIDEO. */
			}
