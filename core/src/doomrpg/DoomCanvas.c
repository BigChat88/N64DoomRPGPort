...
/* The password/code prompt normally needs a keyboard for the digits.  The
 * gamepad path (up/down spin the current digit, right/action commits it) is
 * also used by the N64 port, which has no keyboard but sets padDigitEntry. */
#define PAD_DIGIT_ENTRY	(sdlController.gGameController || sdlController.padDigitEntry)

/* N64 port: scanlines to keep clear at the top and bottom of the play area so
 * TV overscan does not crop the HUD (the mug face / health digits sit flush
 * against the last line otherwise).  The whole display rect -- 3D view, HUD and
 * menus -- is drawn relative to displayRect, so shrinking it here re-centres
 * everything into the safe area.  ~8 suits a CRT; drop to 0 for an emulator on
...
DoomCanvas_t* DoomCanvas_init(DoomCanvas_t* doomCanvas, DoomRPG_t* doomRpg) // 0x12A20
{
	...
	doomCanvas->musicEnabled = true; // N64 port: default per-level music on
...
void DoomCanvas_setupmenu(DoomCanvas_t* doomCanvas, boolean notdrawLoading)
{
	...
	if (notdrawLoading == false) {
		...
		if (doomCanvas->skipIntro) {
			...
			// N64 port: audio is on by default -- skip the "Enable sounds?"
			// prompt and go straight to the main menu with music playing.
			Sound_playSound(doomCanvas->doomRpg->sound, 5040, SND_FLG_LOOP | SND_FLG_STOPSOUNDS | SND_FLG_ISMUSIC, 5);
			MenuSystem_setMenu(doomCanvas->doomRpg->menuSystem, MENU_MAIN);
		}
	}
...
void DoomCanvas_closeDialog(DoomCanvas_t* doomCanvas)
{
	...
	/* [n64 port] Closing the level-up status box ends the level-up theme and
	 * hands playback back to the per-level background music, like the original
	 * game.  No-op when no level-up theme is playing. */
	PD_MusicStingerStop();
...
void DoomCanvas_dialogState(DoomCanvas_t* doomCanvas)
{
	...
	if (doomCanvas->state == ST_DIALOGPASSWORD) {
		...
		if (strLen < (int)SDL_strlen(doomCanvas->strPassCode))
		{
			if (strblink) {
				...
				doomCanvas->strPassCode[strLen] = PAD_DIGIT_ENTRY ? doomCanvas->passInput : ' ';
			}
		}
	}
	...
	if (doomCanvas->numDialogLines > 4) {
		if (doomCanvas->currentDialogLine + 4 == doomCanvas->numDialogLines) {
			DoomCanvas_drawScrollBar(doomCanvas, doomCanvas->SCR_CX + 64, doomCanvas->displayRect.h - 54, 53, doomCanvas->currentDialogLine, doomCanvas->numDialogLines, doomCanvas->numDialogLines);
		}
		else {
			DoomCanvas_drawScrollBar(doomCanvas, doomCanvas->SCR_CX + 64, doomCanvas->displayRect.h - 54, 53, doomCanvas->currentDialogLine, doomCanvas->currentDialogLine + 4, doomCanvas->numDialogLines + 4);
		}
	}
}
...
void DoomCanvas_drawScrollBar(DoomCanvas_t* doomCanvas, int xRight, int y, int totalHeight, int i3, int i4, int i5)
{
	int i6 = i4 - i3;
	if (i6 != 0) {
		...
		DoomCanvas_drawImageSpecial(doomCanvas, &doomCanvas->doomRpg->menuSystem->imgArrowUpDown, 0, 0, 7, 7, 0, xRight, y, 9);
		DoomCanvas_drawImageSpecial(doomCanvas, &doomCanvas->doomRpg->menuSystem->imgArrowUpDown, 0, 7, 7, 7, 0, xRight, y + totalHeight, 10);

		DoomRPG_setColor(doomCanvas->doomRpg, 0x888888);
		DoomRPG_fillRect(doomCanvas->doomRpg, xRight - 7, y + 7, 7, totalHeight - 14);

		DoomRPG_setColor(doomCanvas->doomRpg, 0xDDDDDD);
		DoomRPG_fillRect(doomCanvas->doomRpg, xRight - 7, y + barOffset_y, 7, barHeight);

		DoomRPG_setColor(doomCanvas->doomRpg, 0x000000);
		DoomRPG_drawRect(doomCanvas->doomRpg, xRight - 7, y + barOffset_y, 6, barHeight - 1);
		DoomRPG_drawRect(doomCanvas->doomRpg, xRight - 7, y, 6, totalHeight - 1);
	}
...
void DoomCanvas_handlePasswordEvents(DoomCanvas_t* doomCanvas, int i)
{
	...
	if (doomCanvas->passwordTime == 0) {
		...
		else if (PAD_DIGIT_ENTRY && (key == TURNRIGHT || key == SELECT)) {
		...
		else if (PAD_DIGIT_ENTRY && key == MOVEFORWARD) {
			doomCanvas->passInput++;
			if (doomCanvas->passInput > '9') {
				doomCanvas->passInput = '0';
			}
		}
		else if (PAD_DIGIT_ENTRY && key == MOVEBACK) {
			doomCanvas->passInput--;
...
boolean DoomCanvas_loadMedia(DoomCanvas_t* doomCanvas)
{
	...
	if (Render_beginLoadMap(doomCanvas->render, doomCanvas->loadMapID))
	{
		...
		if (Render_beginLoadMapData(doomCanvas->render))
		{
			...
			doomCanvas->numEvents = 0;
			doomCanvas->isUpdateView = true;
			/* [n64 port] start this level's streamed background music */
			PD_LevelMusicEnable(doomCanvas->musicEnabled);
			PD_LevelMusicForMap(doomCanvas->loadMapID);
			DoomCanvas_setState(doomCanvas, ST_PLAYING);
	...
	if (doomCanvas->renderOnly) {
		...
		if (game->activeSprites || !game->monstersTurn || doomCanvas->openDoorsCount != 0 || Game_updateMonsters(game))
		{
		  ...
		  //    without player movement updates intervening.
			...
			PD_PerfBegin(PD_PERF_HUD);
			Hud_drawTopBar(doomCanvas->hud);
			Hud_drawBottomBar(doomCanvas->hud);
			Hud_drawEffects(doomCanvas->hud);
			PD_PerfEnd(PD_PERF_HUD);
...
void DoomCanvas_renderOnlyState(DoomCanvas_t* doomCanvas)
{
	...
	PD_PerfBegin(PD_PERF_RENDER);
	Render_render(doomCanvas->render, doomCanvas->viewX, doomCanvas->viewY, doomCanvas->viewZ, doomCanvas->viewAngle);
	PD_PerfEnd(PD_PERF_RENDER);
	DoomCanvas_invalidateRectAndUpdateView(doomCanvas);
	DoomCanvas_drawRGB(doomCanvas);
...
void DoomCanvas_run(DoomCanvas_t* doomCanvas)
{
	...
	/* [n64 port] Phase-0 performance overlay -- toggled by r_perf in the
	 * Developer menu.  Rolling 30-frame averages from pd_perf.c, in ms. */
	if (doomCanvas->perfOverlay && (doomCanvas->state != ST_MENU))
	{
		const PD_PerfStats* ps = PD_PerfStatsGet();
		int y = doomCanvas->doomRpg->hud->statusTopBarHeight;
		char line[40];
		...
		snprintf(line, sizeof(line), "fps %u.%02u  tot %u.%ums",
			ps->fps_x100 / 100, ps->fps_x100 % 100,
			ps->us[PD_PERF_TOTAL] / 1000, (ps->us[PD_PERF_TOTAL] % 1000) / 100);
		DoomCanvas_drawString2(doomCanvas, line, 0, y, 0, -1);

		snprintf(line, sizeof(line), "rnd %u.%u  blt %u.%u  hud %u.%u",
			ps->us[PD_PERF_RENDER] / 1000, (ps->us[PD_PERF_RENDER] % 1000) / 100,
			ps->us[PD_PERF_BLIT]   / 1000, (ps->us[PD_PERF_BLIT]   % 1000) / 100,
			ps->us[PD_PERF_HUD]    / 1000, (ps->us[PD_PERF_HUD]    % 1000) / 100);
		DoomCanvas_drawString2(doomCanvas, line, 0, y + 12, 0, -1);

		snprintf(line, sizeof(line), " fc %u.%u  bsp %u.%u",
			ps->us[PD_PERF_FLOORCEIL] / 1000, (ps->us[PD_PERF_FLOORCEIL] % 1000) / 100,
			ps->us[PD_PERF_BSP]       / 1000, (ps->us[PD_PERF_BSP]       % 1000) / 100);
		DoomCanvas_drawString2(doomCanvas, line, 0, y + 24, 0, -1);

		snprintf(line, sizeof(line), " wlk %u.%u wal %u.%u spr %u.%u",
			ps->us[PD_PERF_WALK]    / 1000, (ps->us[PD_PERF_WALK]    % 1000) / 100,
			ps->us[PD_PERF_WALLS]   / 1000, (ps->us[PD_PERF_WALLS]   % 1000) / 100,
			ps->us[PD_PERF_SPRITES] / 1000, (ps->us[PD_PERF_SPRITES] % 1000) / 100);
		DoomCanvas_drawString2(doomCanvas, line, 0, y + 36, 0, -1);

		snprintf(line, sizeof(line), "snd %u.%ums",
			ps->us[PD_PERF_SOUND] / 1000, (ps->us[PD_PERF_SOUND] % 1000) / 100);
		DoomCanvas_drawString2(doomCanvas, line, 0, y + 48, 0, -1);
	}
...
void DoomCanvas_setState(DoomCanvas_t* doomCanvas, int stateNum)
{
	...
	else if (stateNum == ST_MENU)
	{
		...
		{
			/* [n64 port] opening the in-game menu should be silent
...
void DoomCanvas_vibrate(DoomCanvas_t* doomCanvas, int i)
{
	...
	// N64 port: drive the Rumble Pak directly (the SDL paths above are stubs).
	PD_Rumble(i);
}
...
void DoomCanvas_startup(DoomCanvas_t* doomCanvas)
{
	...
	// N64 port: reserve an overscan-safe margin top and bottom (see PD_SAFE_AREA_Y).
	displayH -= 2 * PD_SAFE_AREA_Y;
