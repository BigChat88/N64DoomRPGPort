...
unsigned char* readZipFileEntry(const char* name, zip_file_t* zipFile, int* sizep)
{
	...
	if (entry == NULL) {
		/* [n64 port] Milestone 1 ships only the graphic/level assets in
		 * DoomRPG.zip; the PMD sound bank is not converted yet.  Return NULL
		 * for a missing entry instead of aborting -- the no-op audio shims
		 * tolerate it.  Revert once the sound pipeline lands. */
		*sizep = 0;
		return NULL;
	}
