...
void Render_free(Render_t* render, boolean freePtr)
{
	...
	PD_FbFree(render->framebuffer);   /* [n64 port] uncached alloc */
...
int Render_startup(Render_t* render)
{
	...
	render->framebuffer = PD_FbAlloc(render->pitch * h);   /* [n64 port] uncached */
...
/*
 * N64 port: the phone build's map dialogs point at keypad buttons ("the # key",
 * "the 0 key", "the * and 7 buttons", ...).  Rewrite those exact phrases to N64
 * button names as the map string table loads.  Only these specific substrings
 * are touched, so puzzle codes ("code 042"), the terminal shell prompts
 * ("# scp ..."), the plain English word "OK", and the deliberately corrupted
 * text are left alone.  Returns a (possibly reallocated) string; the original
 * is freed.  '|' is the engine's line break -- kept where the phrase spans two
 * lines so the wrap does not change.
 */
static char* Render_n64Buttonize(char* s)
{
	static const char* const subs[][2] = {
		{ "the # key",                    "the B button" },
		{ "the # button",                 "the B button" },
		{ "the 0 key",                    "the START button" },
		{ "the 0 button",                 "the START button" },
		{ "the 0|key",                    "the START|button" },
		/* prev/next weapon: * and 7 -> L and R */
		{ "the *|and 7 keys",             "the L|and R keys" },
		{ "the * and 7 keys",             "the L and R keys" },
		{ "the * and 7|buttons",          "the L and R|buttons" },
		{ "the * and 7 buttons",          "the L and R buttons" },
		/* skip a combat turn: hold A + Z */
		{ "turn by pressing|the 9 button", "turn by holding|A + Z" },
		{ "the 9 button",                 "A + Z" },
		/* OK (select) -> A, only where it names the button */
		{ "the OK|button",                "the A|button" },
		{ "the OK button",                "the A button" },
	};
	int k;

	for (k = 0; k < (int)(sizeof(subs) / sizeof(subs[0])); k++) {
		const char* from = subs[k][0];
		const char* to = subs[k][1];
		size_t flen = strlen(from);
		size_t tlen = strlen(to);
		char* hit;

		while ((hit = strstr(s, from)) != NULL) {
			size_t head = (size_t)(hit - s);
			size_t tail = strlen(hit + flen);
			char* ns = SDL_malloc(head + tlen + tail + 1);

			memcpy(ns, s, head);
			memcpy(ns + head, to, tlen);
			memcpy(ns + head + tlen, hit + flen, tail + 1);
			SDL_free(s);
			s = ns;
		}
	}
...
boolean Render_beginLoadMapData(Render_t* render)
{
	...
	if (numStrings > 0) {
		...
		while (render->mapStringCount < numStrings) {
			...
			render->mapStringsIDs[render->mapStringCount] =
				Render_n64Buttonize(render->mapStringsIDs[render->mapStringCount]);
...
void Render_render(Render_t* render, int viewx, int viewy, int viewz, unsigned int viewangle)
{
	...
	PD_PerfBegin(PD_PERF_FLOORCEIL);
	...
	PD_PerfEnd(PD_PERF_FLOORCEIL);
	render->horizonFrameTime = DoomRPG_GetTimeMS();

	PD_PerfBegin(PD_PERF_BSP);
	if (render->skipBSP == 0) {
