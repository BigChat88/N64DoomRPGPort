...
void Player_nextLevel(Player_t* player)
{
	...
	if (player->doomRpg->doomCanvas->state != ST_MENU) {
		/* [n64 port] Play the original 5043 "level up" theme once, paired with
		 * the "Level up!" status box -- closing the box stops it
		 * (DoomCanvas_closeDialog -> PD_MusicStingerStop) and the streamed
		 * per-level music resumes.  Gated on the same !ST_MENU check as the
		 * box: a level-up done from the debug menu shows no box, so it must
		 * not start the theme (nothing would ever stop it).  (The old
		 * SND_FLG_ISMUSIC path looped 5043 forever and permanently closed the
		 * streamed level track -- see Sound_playLevelUpMusic / PD_MusicStinger.) */
		Sound_playLevelUpMusic(player->doomRpg->sound);
