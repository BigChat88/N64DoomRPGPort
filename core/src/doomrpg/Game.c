...
#define CONFIG_VERSION 24 // N64 port: + per-level music toggle
...
void Game_loadConfig(Game_t* game)
{
	...
	if (rw) {
		...
		if (version == CONFIG_VERSION) {
			...
			// N64 port: per-level background music toggle
			boolData = File_readByte(rw);
			if (game) {
				game->doomRpg->doomCanvas->musicEnabled = boolData != 0 ? true : false;
				PD_LevelMusicEnable(game->doomRpg->doomCanvas->musicEnabled);
			}
...
void Game_saveConfig(Game_t* game, int num)
{
	...
	// N64 port: per-level background music toggle
	File_writeByte(rw, game->doomRpg->doomCanvas->musicEnabled);
